...
const AdamJoin: React.FC = () => {
  return (
    <div id="adam-join" className="w-full h-full flex flex-col items-center justify-center text-center p-8 bg-transparent relative">
      <div className="relative z-10">
        ...
        <p className="mt-4 text-adam-light-gray font-light max-w-xl mx-auto">
          Contribute to the ADAM Protocol. Help build, propose, and ratify the constitution for decentralized, impact-driven systems.
        </p>
        <div className="mt-10">
            <a href="#" className="border border-adam-blue text-adam-blue font-light py-4 px-12 rounded-sm text-md uppercase tracking-[0.2em] hover:bg-adam-blue hover:text-black hover:shadow-glow-blue transition-all duration-300 group relative">
                <span className="absolute -inset-1 rounded-sm bg-adam-blue opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-300"></span>
                Join The Community
            </a>
        </div>
      </div>
       <footer className="absolute bottom-8 text-center text-xs text-gray-700 font-light tracking-widest w-full">
         <div className="flex justify-center space-x-6 mb-4">
          <a href="#" className="text-gray-600 hover:text-adam-blue transition-colors duration-300" aria-label="ADAM Protocol Twitter">
            <span className="sr-only">Twitter</span>
            <TwitterIcon className="h-5 w-5" />
          </a>
          <a href="#" className="text-gray-600 hover:text-adam-blue transition-colors duration-300" aria-label="ADAM Protocol Discord">
            <span className="sr-only">Discord</span>
            <DiscordIcon className="h-5 w-5" />
          </a>
        </div>
        <p>&copy; {new Date().getFullYear()} ADAM PROTOCOL. ALL RIGHTS RESERVED.</p>
      </footer>
    </div>
  );
};

export default AdamJoin;